...
void AddActuatorsToPlant(multibody::MultibodyPlant<double>& plant) {
  if (plant.num_actuated_dofs() == 0) {  // Get Number of Actuated DOFs
    std::cout << std::fixed
              << std::setprecision(
                     4);  // Set fixed-point notation with 4 decimal places
    std::cout
        << "-------------------------------------------------------------------"
           "-----------------------------------------------------\n";
    std::cout << "                                 Software Added Joint "
                 "Actuator Information Table \n";
    std::cout
        << "-------------------------------------------------------------------"
           "-----------------------------------------------------\n";
    std::cout << std::setw(5) << std::left << "Idx" << std::setw(35)
              << "Joint Name" << std::setw(20) << "Effort Limit (Nm)"
              << std::setw(25) << "Velocity Limit (rad/s)" << std::setw(15)
              << "Pos Min (rad)" << std::setw(15) << "Pos Max (rad)"
              << std::setw(15) << "Roll (rad)" << std::setw(15) << "Pitch (rad)"
              << std::setw(15) << "Yaw (rad)"
              << "\n";
    std::cout << "-------------------------------------------------------------"
                 "----------------------------------------------------------\n";
    ...
    for (int i = 0; i < plant.num_joints(); ++i) {
      ...
      if (joint.num_positions() >= 1 &&
          joint.num_velocities() >= 1) {  // Only consider 1-DOF joints
        const auto& actuator =
        ...
        std::cout << std::setw(5) << std::left << i << std::setw(35)
                  << actuator.name()  // Print actuator name
                  << std::setw(20) << effort_limit << std::setw(25)
                  << velocity_limit << std::setw(15) << pos_min << std::setw(15)
                  << pos_max << std::setw(15) << rpy_PC.roll_angle()
                  << std::setw(15) << rpy_PC.pitch_angle() << std::setw(15)
                  << rpy_PC.yaw_angle() << "\n";
      }
    }
    ...
    std::cout << "-------------------------------------------------------------"
                 "-----------------------------------------------\n";
    std::cout << plant.num_actuated_dofs() << " actuators added to the system."
              << std::endl;
    std::cout << "-------------------------------------------------------------"
                 "-----------------------------------------------\n";
  }
