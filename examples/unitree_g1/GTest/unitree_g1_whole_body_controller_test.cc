...
GTEST_TEST(UnitreeG1WholeBodyControllerTest, PDSubsystemIntegration) {
  ...
  UnitreeG1WholeBodyController controller(plant, kp, kd);
  ...
  EXPECT_NO_THROW(controller.GetInputPort("pd_controller_input"));
  EXPECT_NO_THROW(controller.GetOutputPort("pd_controller_output"));
