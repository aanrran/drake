...
PD_Controller<T>::PD_Controller(
  ...
  */
  ...
  auto gravity_comp = builder.template AddSystem<InverseDynamics<T>>(&plant_, InverseDynamics<T>::kGravityCompensation);
  // ✅ Use MatrixGain to transform gravity compensation output
  Eigen::MatrixXd gravity_selection = Eigen::MatrixXd::Zero(plant_.num_actuated_dofs(), plant_.num_velocities());
for (int i = 0; i < plant_.num_actuated_dofs(); ++i) {
    gravity_selection(i, plant_.num_velocities() - plant_.num_actuated_dofs() + i) = 1.0;  // Select only actuated joints
}

auto gravity_selector = builder.template AddSystem<drake::systems::MatrixGain>(gravity_selection);

// ✅ Connect Gravity Compensation Output to the Selector
builder.Connect(gravity_comp->get_output_port(0), gravity_selector->get_input_port());
  ...
  builder.Connect(gravity_selector->get_output_port(), adder->get_input_port(1));
  builder.Connect(spring->get_output_port(0), adder->get_input_port(2));
  builder.Connect(damper->get_output_port(0), adder->get_input_port(0));

  // Exposes the estimated state port.
  // Connects the estimated state to the gravity compensator.
  input_port_index_estimated_state_ = builder.ExportInput(gravity_comp->get_input_port_estimated_state(), "estimated_state");

  // Connects the estimated state to the spring.
  builder.ConnectInput(input_port_index_estimated_state_, spring->get_input_port_estimated_state());
